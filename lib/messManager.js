...
function MessageManager (dba){
  ...
  this._manageGrouping = async (mess, policie) => {
    return new Promise((resolve, reject) => {
      ...
      //récupération des messages ayant le même message + date dans le cadre + même priorité + même origine.
      this._getMess(mess.typeMessage,
        {"message" : mess.message, "priority" : mess.priority, "origine" : mess.origine, "date" : {"$gte" : dateN}}).then((messPrec) => {
  ...
  this.getMessages = async (typeMessage, filter) => {
    return new Promise((resolve, reject) => {
      ...
      if (filter.dateD != undefined){
        if (filt.date == undefined) filt.date = {};
